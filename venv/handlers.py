...
@dp.message_handler(commands=['start', 'старт'])        # отлавливает сообщения с командой "старт"   
async def mes_start(messadge: types.Message):
    name = messadge.from_user.first_name
    await messadge.answer(f'{name}, привет! Сегодyя сыграем с тобой в конфеты! Для начала игры введи команду /new_game. '
                          f'Для настройки конфет введи команду /set и укажи количество конфет' )
    print('Вам пришло сообщение')
    print(message.from_user.id)


@dp.message_handler(text=['help'])                      # на вход принимает текст, а не команду
async def mes_help(messadge: types.Message):
    await messadge.answer('Бог может')
